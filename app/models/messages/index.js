...
exports.send = async (reqParams) => {
 try {
  ...
  const notificationParams = {
   ...
   data: { type: "2", action: "1", data: JSON.stringify({ ref_id: msg_id, sender_id: sender_id, receiver_id: receiver_id, username: username }) }
  }
...
exports.update = async (reqParams) => {
 try {
  ...
  if ("message_status" in reqParams) {
   ...
   const io = getIO()
   const socketId = getSocketIdFromUserId(receiver_id)
   if (socketId) {
    io.to(socketId).emit("update_msg_status", { _id: msg_id, sender_id: reqParams["user_id"], receiver_id: receiver_id, message_status: reqParams["message_status"] })
   }
  }
